...
| `Chuying Proxy Play<版本>-Forge-1.20.1-linux.jar` | Linux（x86-64, AVX2） |
| `Chuying Proxy Play<版本>-Forge-1.20.1-macos.jar` | macOS（Apple Silicon） |

> 引擎首次运行时自动解压到 `config/chuying/engines/`，无需手动配置。

> 需要 NeoForge 1.21.1 或 Fabric 1.20.1 版本？请到 Releases 选择对应分支 / 发行页的 jar。

## 依赖

- Forge `47+`（Minecraft 1.20.1）
- [Touhou Little Maid](https://modrinth.com/mod/touhou-little-maid) ≥ `1.5.0` —— 客户端与服务端都需要（棋盘来自它）
- Cloth Config 已 JarJar 内嵌，**无需**另行安装
